...
{
    /// <summary>
    /// An action filter that validates the arguments of an action method using FluentValidation.
    /// </summary>
    /// <remarks>
    /// This filter, which can be applied to a controller action method, an entire controller or all
    /// the controllers in an ASP.NET Core Web API project, validates each action argument using the 
    /// appropriate <see cref="IValidator{T}"/> implementation registered in the dependency injection 
    /// container. If no <see cref="IValidator{T}"/> implementation is registered in the DI container,
    /// no validation is done. Otherwise, each argument of the action method is validated against the
    /// rules defined in the <see cref="IValidator{T}"/> implementation. If an action argument is null 
